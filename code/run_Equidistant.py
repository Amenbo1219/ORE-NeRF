...
from datasets.nerf_dataloader_dx import NeRFRayDataset
...
from helpers.run_nerf_helpers_sp_axis_dx import *
...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    if viewdirs is not None:
        if viewdirs.ndim == 2:  # viewdirs = [N_rays, 7]
            # Expand to match sample数
...
def train():
    ...
    if args.render_only:
        print('RENDER ONLY')
        dataset = NeRFRayDataset(args.datadir, 'train')
        ...
        with torch.no_grad():
            if args.render_test:
                test_dataset = NeRFRayDataset(args.datadir, 'test')
            ...
            with torch.amp.autocast(device_type=device_type,enabled=args.amp):
                        rgbs, _ = render_path(render_poses, [H, W], K, args.chunk, render_kwargs_test, 
                                 gt_imgs=images, savedir=testsavedir, 
                                 render_factor=args.render_factor, mask=mask)
    ...
    test_dataset = NeRFRayDataset(args.datadir, mode='test')
    ...
    for i in pbar:
        ...
        with torch.amp.autocast(device_type=device_type, enabled=args.amp):
            ...
            optimizer.zero_grad()
            if hasattr(train_dataset, 'mask') and train_dataset.mask is not None:
                pixel_i = pixel_coords[:, 0].long()  
                pixel_j = pixel_coords[:, 1].long()  
                
                mask_values = np.array([train_dataset.mask[j, i] for i, j in zip(pixel_i, pixel_j)], dtype=bool)
                valid_pixels = torch.tensor(mask_values, device=device)
        ...
        if i % (Each_epoch_N_iters * args.i_video)==0 and i > 0:
            ...
            with torch.no_grad():
                ...
                K = test_dataset.K
                mask = test_dataset.mask if hasattr(test_dataset, 'mask') else None
                render_poses = torch.Tensor(test_dataset.render_poses).to(device)
                with torch.no_grad():
                    images = None
                    testsavedir = os.path.join(args.basedir, args.expname, 'renderonly_{}_{:06d}'.format('test' if args.render_test else 'path', start))
                    os.makedirs(testsavedir, exist_ok=True)
            ...
            print('Done rendering video, saving', rgbs.shape)
            print('Video saved at', os.path.join(testsavedir, 'video.mp4'))
        ...
        if  i % (Each_epoch_N_iters * args.i_testset) == 0 and i > 0:
            ...
            with torch.no_grad():
                for idx, batch in tqdm(enumerate(test_loader), total=len(test_loader), desc="Rendering test set"):
                    ...
                    rgb, _, _, _ = render(H, W, K, chunk=args.chunk, rays=batch_rays, **render_kwargs_test)
